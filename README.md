...
capability graph, and authorization is determined by evaluating that graph
locally; without network access, shared state, or trusted infrastructure.
...
- **Interoperability** - No ecosystem, app or company lock-in. Works in any
  JWT-friendly system.
- **Deterministic evaluation** - Given a finite set of tokens, authorization
  decisions are a deterministic local computation, not a runtime query to an
  external service.

This means authorization decisions depend only on the presented tokens and the
verification algorithm, not on timing, network availability, or external state.
...
The following documents define the Vouchsafe system and its underlying
ZI-CG-based trust model:
...
vouchsafe tokens. That said, some things (such as trust graph evaluation) are 
